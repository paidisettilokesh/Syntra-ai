...
async def test_orchestrator_notification_failure_isolation():
    ...
    mock_ai = MagicMock()
    mock_ai.analyze = MagicMock(return_value=asyncio.sleep(0, result=AnalysisResult(
        category="Urgent", importance_score=9, summary="sum", reasoning="reason", action_required=True, confidence_score=0.9
    )))

    mock_verification = MagicMock()
    mock_verification.verify_email = MagicMock(return_value=asyncio.sleep(0, result=VerificationResult(
        is_legitimate=True, status="Legitimate", confidence=100.0, risk_score=0, risk_level="Low", decision="Sent", reason="OK", threats=[], triggered_rules=[]
    )))
